...
// Server lists the authstorage service endpoint HTTP handlers.
type Server struct {
	Mounts []*MountPoint
	Insert http.Handler
	Delete http.Handler
...
// New instantiates HTTP handlers for all the authstorage service endpoints.
func New(
	e *authstorage.Endpoints,
	mux goahttp.Muxer,
	...
	return &Server{
		Mounts: []*MountPoint{
			{"Insert", "POST", "/storage/auth/insert"},
			{"Delete", "DELETE", "/storage/auth/delete/{id}"},
			{"Get", "GET", "/storage/auth/get/{id}"},
		},
...
func (s *Server) Service() string { return "authstorage" }
...
// Mount configures the mux to serve the authstorage endpoints.
...
// MountInsertHandler configures the mux to serve the "authstorage" service
// "insert" endpoint.
func MountInsertHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("POST", "/storage/auth/insert", f)
}

// NewInsertHandler creates a HTTP handler which loads the HTTP request and
// calls the "authstorage" service "insert" endpoint.
func NewInsertHandler(
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		ctx = context.WithValue(ctx, goa.ServiceKey, "authstorage")
...
// MountDeleteHandler configures the mux to serve the "authstorage" service
// "delete" endpoint.
func MountDeleteHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("DELETE", "/storage/auth/delete/{id}", f)
}

// NewDeleteHandler creates a HTTP handler which loads the HTTP request and
// calls the "authstorage" service "delete" endpoint.
...
// MountGetHandler configures the mux to serve the "authstorage" service "get"
// endpoint.
