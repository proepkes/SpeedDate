...
import (
	...
	cli "github.com/proepkes/speeddate/src/storagesvc/gen/http/cli/storager"
	goahttp "goa.design/goa/http"
)

func main() {
	var (
		addr = flag.String("url", "http://:8001", "`URL` to service host")
...
func usage() {
	fmt.Fprintf(os.Stderr, `%s is a command line client for the storager server.

Usage:
    %s [-url URL][-timeout SECONDS][-verbose|-v] SERVICE ENDPOINT [flags]

    -url URL:    specify service URL (http://:8001)
