...
import (
	...
	cli "github.com/proepkes/speeddate/src/mmsvc/gen/http/cli/matchmaker"
	goahttp "goa.design/goa/http"
)

func main() {
	var (
		addr = flag.String("url", "http://:8003", "`URL` to service host")
...
func usage() {
	fmt.Fprintf(os.Stderr, `%s is a command line client for the matchmaker server.

Usage:
    %s [-url URL][-timeout SECONDS][-verbose|-v] SERVICE ENDPOINT [flags]

    -url URL:    specify service URL (http://:8003)
